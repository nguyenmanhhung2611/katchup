...
<body >
	...
	<?php echo form_open_multipart('admin/executeUpdateDocument/'.$document[TAI_LIEU_COL_MA_TAI_LIEU]);?>
		...
		Hình Ảnh: <?php echo DOCUMENT_PAGE_RESOURCE_IMAGE_PATH.$document[TAI_LIEU_COL_HINH_ANH]; ?>
		<img src="<?php echo DOCUMENT_PAGE_RESOURCE_IMAGE_PATH.$document[TAI_LIEU_COL_HINH_ANH]; ?>" width="50" height="60" />
		...
		<select name="danhMuc" >
			...
			foreach ($categoryList as $cat) {
				?>
				<option value="<?php echo $cat[DANH_MUC_COL_MA_DANH_MUC]; ?>" <?php if ($document[TAI_LIEU_COL_MA_DANH_MUC] === $cat[DANH_MUC_COL_MA_DANH_MUC]) echo "selected"; ?>><?php echo $cat[DANH_MUC_COL_TEN_DANH_MUC]; ?></option>
				<?php
			}
			?>			
		</select><br/>
